...
public class Transaction {
	...
	List<Item> cacheList;
	static List<threadFifoClass> threadFifo;
	
	synchronized threadFifoClass getMessage(String myDomaine){
		threadFifoClass tc = null;
		if (threadFifo.size() > 0){
			Iterator<threadFifoClass> it = threadFifo.iterator();
			while(it.hasNext()){
				threadFifoClass tcMessage = it.next();
				if (tcMessage.forDomainName.equals(myDomaine)){
					tc = tcMessage;
					it.remove();
					break;
				}
...
//		ClientConfiguration config = new ClientConfiguration();
		...
		//System.out.println(System.getProperty("user.dir"));
	       ...
	       try {
			sdb = new AmazonSimpleDBClient(new PropertiesCredentials
			    		(new File( Conf.getCredentialFilePath())));
		} catch (IOException e) {
	...
	public void receiveProduitFromDb(String domainName){
		
	}
	public void sendProduitToDb(String domainName){
